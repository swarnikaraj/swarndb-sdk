...
from swarndb.remote_thrift.types import build_result, logic_type_to_dtype
from swarndb.utils import binary_exp_to_paser_exp
from swarndb.common import SwarndbException, SparseVector, Array, VEC
from swarndb.errors import ErrorCode

# ---------------------------
# Parsed Expression Utilities
# ---------------------------
...
def parsed_expression_to_string(expr: ttypes.ParsedExpr) -> str:
    ...
    if expr_type.column_expr:
        if expr_type.column_expr.column_name:
            return ".".join(expr_type.column_expr.column_name)
    ...
    if expr_type.between_expr:
        ...
        return f"between({value_str}, {upper_bound_str}, {lower_bound_str})"
...
def search_to_string(search_expr: ttypes.SearchExpr) -> str:
    ...
    raise SwarndbException(ErrorCode.INVALID_EXPRESSION, "Invalid search expression")
...
def traverse_conditions(cons, fn: Optional[Callable] = None) -> ttypes.ParsedExpr:
    ...
    if isinstance(cons, exp.Binary):
        ...
        function_expr.function_name = binary_exp_to_paser_exp(cons.key.lower())
        arguments = []
        ...
        match cons.alias_or_name:
            case "_row_id":
                ...
                return ttypes.ParsedExpr(type=expr_type)
            ...
            case _:
                ...
                column_expr.star = (cons.alias_or_name == "*")
        ...
        if cons.is_int:
            ...
            raise SwarndbException(ErrorCode.INVALID_EXPRESSION, f"Unknown literal type: {cons}")
        parser_expr_type = ttypes.ParsedExprType()
        parser_expr_type.constant_expr = constant_expr
        parsed_expr.type = parser_expr_type
        return parsed_expr
        ...
        left_expr = parse_expr(cons.args['this'])
        arguments = []
        for arg in cons.args['expressions']:
            if arg:
                arguments.append(parse_expr(arg))
        ...
        raw_in = cons.args['this']
        left_expr = parse_expr(raw_in.args['this'])
        arguments = []
        ...
        raise SwarndbException(ErrorCode.INVALID_EXPRESSION, f"unknown condition type: {cons}")
...
def parse_expr(expr) -> ttypes.ParsedExpr:
    try:
        ...
        if isinstance(expr, exp.Star):
            ...
            raise SwarndbException(ErrorCode.INVALID_EXPRESSION, f"unknown expression type: {expr}") from e
...
def get_remote_constant_expr_from_python_value(value) -> ttypes.ConstantExpr:
    ...
    match value:
        ...
        case _:
            raise SwarndbException(ErrorCode.INVALID_EXPRESSION, f"Invalid constant type: {type(value)}")
    return constant_expression

identifier_limit = 65536

def check_valid_name(name: Any, name_type: str = "Table") -> None:
    if not isinstance(name, str):
        raise SwarndbException(ErrorCode.INVALID_IDENTIFIER_NAME,
    ...
    if name is None or name.isspace() or name == '' or name == ' ' or name.isdigit():
        raise SwarndbException(ErrorCode.INVALID_IDENTIFIER_NAME, f"Invalid {name_type} name: {name}")
...
def select_res_to_polars(res) -> Tuple[pl.DataFrame, Any]:
    df_dict = {}
    ...
    return pl.from_pandas(pd.DataFrame(df_dict)), extra_result

def get_constant_expr(column_info: dict) -> ttypes.ConstantExpr:
    default = column_info.get("default")
    if default is None:
        return ttypes.ConstantExpr(literal_type=ttypes.LiteralType.Null)
    else:
        ...
        match constraint:
            case "null":
                if ttypes.Constraint.Null not in res:
                    ...
                    raise SwarndbException(ErrorCode.INVALID_CONSTRAINT_TYPE, f"Duplicated constraint: {constraint}")
            case "not null":
            ...
            case "unique":
                if ttypes.Constraint.Unique not in res:
                    ...
                    raise SwarndbException(ErrorCode.INVALID_CONSTRAINT_TYPE, f"Duplicated constraint: {constraint}")
            case _:
                raise SwarndbException(ErrorCode.INVALID_CONSTRAINT_TYPE, f"Unknown constraint: {constraint}")
    return res

def get_embedding_element_type(element_type: str) -> ttypes.ElementType:
    match element_type:
        case "bit":
        ...
        case _:
            raise SwarndbException(ErrorCode.INVALID_EMBEDDING_DATA_TYPE, f"Unknown element type: {element_type}")
...
def get_embedding_type(column_big_info: List[str]) -> ttypes.DataType:
    ...
    match column_big_info[0]:
        ...
        case _:
            raise SwarndbException(ErrorCode.INVALID_DATA_TYPE, f"Unknown data type: {column_big_info[0]}")
...
def get_sparse_type(column_big_info: List[str]) -> ttypes.DataType:
    ...
    if index_type == "int8":
        ...
        raise SwarndbException(ErrorCode.INVALID_EMBEDDING_DATA_TYPE, f"Unknown index type: {index_type}")
...
def get_data_type(column_info: dict) -> ttypes.DataType:
    if "type" not in column_info:
        raise SwarndbException(ErrorCode.NO_COLUMN_DEFINED, "Column definition without data type")
...
def get_data_type_from_column_big_info(column_big_info: List[str]) -> ttypes.DataType:
    column_big_info_first_str = column_big_info[0]
    match column_big_info_first_str:
        case "vector" | "multivector" | "tensor" | "tensorarray":
            return get_embedding_type(column_big_info)
        case "sparse":
            return get_sparse_type(column_big_info)
        case "array":
            if len(column_big_info) < 2:
                raise SwarndbException(ErrorCode.INVALID_DATA_TYPE, "No element type for array!")
        ...
        case _:
            if len(column_big_info) > 1:
                raise SwarndbException(ErrorCode.INVALID_DATA_TYPE, f"Unknown datatype: {column_big_info}, too many arguments")
            ...
            match column_big_info_first_str:
                ...
                case _:
                    raise SwarndbException(ErrorCode.INVALID_DATA_TYPE, f"Unknown datatype: {column_big_info_first_str}")
